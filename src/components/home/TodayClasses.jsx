...
import { ChevronRight, FileText, CheckCircle2, Loader2, Clock, PartyPopper } from 'lucide-react';
import { CustomMapPin } from '../common/CustomMapPin';
import { CustomEyes } from '../common/CustomIcons';
import SkeletonCard from './SkeletonCard';

const TodayClasses = ({ isExamModeActive, todaysExams, todaysClassesWithStatus, allCompleted, todayHoliday, navigate, formatTime12Hour, isDeferredActive }) => {
        ...
        <>
          ...
          {todaysExams.length === 0 ? (
            <div className="flex items-center justify-between py-2">
              ...
              <button onClick={() => navigate('/tools/assignments')} className="flex bg-amber-50 text-amber-700 px-4 py-2 rounded-xl text-xs font-bold hover:bg-amber-100 transition-colors">Add Exam</button>
            </div>
          ) : (
        ...
        <>
          ...
          {todaysClassesWithStatus.length === 0 ? (
            <div className="flex items-center justify-between py-2">
              ...
              <button onClick={() => navigate('/tools/timetable')} className="flex bg-primary-50 text-primary-600 px-4 py-2 rounded-xl text-xs font-bold hover:bg-primary-100 transition-colors">Add Class</button>
            </div>
          ) : allCompleted ? (
            ...
            <div className="space-y-3">
              {todaysClassesWithStatus.slice(0, 3).map((cls, i) => (
                <div key={i} className={`flex items-center gap-4 transition-opacity ${cls.status === 'completed' ? 'opacity-40' : 'opacity-100'}`}>
                  <div className={`w-10 h-10 rounded-2xl flex items-center justify-center flex-shrink-0 ${cls.status === 'completed' ? 'bg-gray-100' : cls.status === 'ongoing' ? 'bg-blue-50 border border-blue-100 shadow-sm' : 'bg-gray-900/5'}`}>
