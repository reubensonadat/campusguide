    ...
    const renderFinance = () => (
        <div className="space-y-4">
            ...
            <Question label="1. Do you currently track your daily spending?">
                <RadioGroup value={answers.finance_tracking} onChange={(val) => updateAnswer('finance_tracking', val)} options={['Yes, religiously', 'Sometimes / Mental note', 'No, never']} />
            </Question>

            {/* UPDATED Q2 */}
            <Question label="2. Would you use a 'Quick Add' widget for fast manual tracking?">
                <RadioGroup value={answers.finance_quick_add} onChange={(val) => updateAnswer('finance_quick_add', val)} options={['Yes, if it takes <5s', 'Maybe', 'No, I\'ll forget']} />
            </Question>

            <Question label="3. Is a 'Daily Budget Limit' alert useful?">
                <RadioGroup value={answers.finance_budget_alert} onChange={(val) => updateAnswer('finance_budget_alert', val)} options={['Essential', 'Nice to have', 'Not needed']} />
            </Question>

            <Question label="4. Would you pay to export expense reports (PDF/Excel)?">
                <RadioGroup value={answers.finance_export} onChange={(val) => updateAnswer('finance_export', val)} options={['Yes', 'No, should be free']} />
            </Question>
    ...
    const renderAcademics = () => (
        <div className="space-y-4">
            ...
            <Question label="6. How do you calculate your GPA currently?">
                <RadioGroup value={answers.academic_gpa_calc} onChange={(val) => updateAnswer('academic_gpa_calc', val)} options={['Manual (Pen & Paper)', 'Another App', 'I don\'t track it']} />
            </Question>

            <Question label="7. Would a 'Dynamic What-If GPA Manager' be helpful?">
                <RadioGroup value={answers.academic_target_gpa} onChange={(val) => updateAnswer('academic_target_gpa', val)} options={['Game changer!', 'Useful', 'Not really']} />
            </Question>

            {/* UPDATED Q8 */}
            <Question label="8. Do you want class reminders sent as Push Notifications?">
                <RadioGroup value={answers.academic_reminders} onChange={(val) => updateAnswer('academic_reminders', val)} options={['Yes, 15 min before', 'Yes, 30 min before', 'No, I\'ll check manually']} />
            </Question>

            <Question label="9. Is OFFLINE access to your timetable essential?">
                <RadioGroup value={answers.academic_offline} onChange={(val) => updateAnswer('academic_offline', val)} options={['Critical (Data is expensive)', 'Preferred', 'I always have data']} />
            </Question>

            <Question label="10. Would you share your schedule with friends?">
                <RadioGroup value={answers.academic_share} onChange={(val) => updateAnswer('academic_share', val)} options={['Yes, definitely', 'Maybe', 'No, private']} />
            </Question>
    ...
    const renderLife = () => (
        <div className="space-y-4">
            ...
            <Question label="11. How do you buy event tickets usually?">
                <RadioGroup value={answers.life_tickets} onChange={(val) => updateAnswer('life_tickets', val)} options={['Physical Agents', 'Shortcode/Mobile Money', 'Online Website']} />
            </Question>

            <Question label="12. Would you trust an in-app Ticket Wallet (QR Code)?">
                <RadioGroup value={answers.life_wallet} onChange={(val) => updateAnswer('life_wallet', val)} options={['Yes, more convenient', 'Maybe', 'I prefer printed/SMS']} />
            </Question>

            <Question label="13. Do you want to see which friends are attending an event?">
                <RadioGroup value={answers.life_friends} onChange={(val) => updateAnswer('life_friends', val)} options={['Yes, social is key', 'Don\'t care']} />
            </Question>

            <Question label="14. How often do you get lost on campus?">
                <RadioGroup value={answers.life_map_freq} onChange={(val) => updateAnswer('life_map_freq', val)} options={['Often (I\'m new)', 'Rarely', 'Never']} />
            </Question>

            {/* UPDATED Q15 */}
            <Question label="15. Would you use the map to find Food spots & Amenities?">
                <RadioGroup value={answers.life_amenities} onChange={(val) => updateAnswer('life_amenities', val)} options={['Yes, very useful', 'Sometimes', 'No, I know places']} />
            </Question>
    ...
    const renderValue = () => (
        <div className="space-y-4">
            ...
            <Question label="16. For PRO features (What-If GPA, Smart Slides, Digital Library & Skill Courses, Course Notes & Slides Manager), what is a fair semester Price?">
                <RadioGroup value={answers.value_price} onChange={(val) => updateAnswer('value_price', val)} options={['GH₵ 25 / semester', 'GH₵ 30 / semester', 'GH₵ 40 / semester', 'GH₵ 50 / semester']} />
            </Question>

            <Question label="17. What is the ONE app you use most for school right now?">
                <TextInput value={answers.value_current_app} onChange={(val) => updateAnswer('value_current_app', val)} placeholder="e.g. WhatsApp, Portal..." />
            </Question>

            {/* UPDATED Q18 */}
            <Question label="18. Would you like to see Verified Student Ads (Safety First)?">
                <RadioGroup value={answers.value_ads} onChange={(val) => updateAnswer('value_ads', val)} options={['Yes, to buy/sell safely', 'Maybe', 'No, no ads please']} />
            </Question>

            {/* UPDATED Q19 */}
            <Question label="19. Preferred In-App Notification Style?">
                <RadioGroup value={answers.value_notify_style} onChange={(val) => updateAnswer('value_notify_style', val)} options={['Push Notifications (Lock screen)', 'Silent In-App Badge', 'None']} />
            </Question>

            <Question label="20. Any final advice for us?">
                <TextInput value={answers.value_open_feedback} onChange={(val) => updateAnswer('value_open_feedback', val)} placeholder="Be honest..." />
            </Question>
