...
function PopupCloseButton({ onClick }: { onClick: () => void }) {
  return (
    <button
      ...
      className="focus-visible:ring-slate-950 hover:bg-slate-100 text-slate-950 absolute top-0.5 right-0.5 z-10 inline-flex size-5 cursor-pointer items-center justify-center rounded-sm transition-colors focus:outline-none focus-visible:ring-2   "
    >
...
function MarkerPopup({
  ...
  return createPortal(
    <div
      className={cn(
        "text-slate-950 relative max-w-62",
...
function MarkerTooltip({
  ...
  return createPortal(
    <div
      className={cn(
        "bg-slate-950 text-white pointer-events-none rounded-md px-2 py-1 text-xs text-balance shadow-md  ",
        "animate-in fade-in-0 zoom-in-95 duration-200 ease-out",
        className,
      )}
    >
  ...
  return (
    <div
      className={cn(
        ...
        "text-slate-950 text-[10px] font-medium ",
