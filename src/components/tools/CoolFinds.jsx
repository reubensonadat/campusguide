...
import { ExternalLink, Search, ArrowRight, Loader2 } from 'lucide-react';
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="bg-white rounded-[24px] p-6 shadow-sm border border-gray-100 flex flex-col md:flex-row gap-4 items-center justify-between">
        <div className="relative w-full md:w-96 flex-shrink-0">
          ...
          <input
            ...
            className="w-full pl-11 pr-4 py-3 bg-gray-50 border-none rounded-xl text-sm font-medium focus:outline-none focus:ring-2 focus:ring-[#002F45]/20 transition-all placeholder:text-gray-400"
          />
        </div>
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-20">
          <Loader2 className="w-8 h-8 text-[#002F45] animate-spin mb-4" />
            ...
            filteredResources.map((resource, idx) => {
              ...
              return (
                <a 
                  ...
                  key={resource.id || idx} 
                  className="group bg-white rounded-[24px] p-6 shadow-sm border border-gray-100 hover:shadow-xl hover:border-[#002F45]/20 transition-all cursor-pointer flex flex-col h-full active:scale-[0.98]"
                >
                  <div className="flex items-start justify-between mb-4">
                    <div className="w-12 h-12 rounded-2xl bg-[#002F45]/5 text-[#002F45] flex items-center justify-center">
                      <Icon size={24} />
                    </div>
                    <div className="w-8 h-8 rounded-full bg-gray-50 flex items-center justify-center group-hover:bg-[#002F45] group-hover:text-white transition-colors text-gray-400">
                      <ArrowRight size={14} className="-rotate-45" />
                    </div>
                  </div>
                  
                  <h3 className="text-lg font-bold text-gray-900 mb-2 group-hover:text-[#002F45] transition-colors line-clamp-1">{resource.title}</h3>
                  <p className="text-sm text-gray-500 font-medium leading-relaxed flex-1">{resource.description}</p>
                  
                  <div className="mt-6 flex items-center gap-2 text-xs font-bold text-gray-400 group-hover:text-[#002F45]/60 transition-colors">
