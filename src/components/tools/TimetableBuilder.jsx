      ...
      <Card>
        <CardHeader>
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
              <CardTitle className="text-indigo-900 dark:text-indigo-400 text-xl flex items-center gap-2">
                <Calendar className="w-5 h-5 text-indigo-500 dark:text-indigo-400" />
                My Timetable
              </CardTitle>
              <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Design your perfect weekly schedule</p>
            </div>
            ...
            <div className="flex w-full sm:w-auto gap-2">
              {!notificationsEnabled && isNotificationSupported() && (
                <Button
                  ...
                  className="flex-1 sm:flex-none border-indigo-200 text-indigo-700 hover:bg-indigo-50 dark:bg-indigo-900/40"
                >
        ...
        <CardContent>
          {showAddForm && (
            <div className="mb-6 bg-gray-50 dark:bg-gray-900 p-6 rounded-2xl border border-gray-100 dark:border-gray-800 animate-in fade-in slide-in-from-top-2">
              <h4 className="font-bold text-gray-900 dark:text-gray-100 mb-4">Add Schedule Item</h4>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="md:col-span-2">
                  <label className="text-xs font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-2 block">Color Tag</label>
          ...
          {/* Desktop View (Table) */}
          <div id="timetable-grid" className="hidden lg:block overflow-x-auto rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900">
            <table className="w-full border-collapse">
              <thead>
                <tr>
                  <th className="border-b border-gray-200 dark:border-gray-700 p-4 bg-gray-50 dark:bg-gray-900/50 text-xs font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest text-left w-24">Time</th>
                  {DAYS_OF_WEEK.map(day => (
                    <th key={day} className="border-b border-l border-gray-200 dark:border-gray-700 p-4 bg-gray-50 dark:bg-gray-900/50 text-xs font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest text-left min-w-[140px]">{day}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {TIME_SLOTS.map(time => (
                  <tr key={time} className="group hover:bg-gray-50 dark:hover:bg-gray-800 dark:bg-gray-900/30 transition-colors">
                    <td className="border-r border-gray-100 dark:border-gray-800 p-3 text-xs font-medium text-gray-400 dark:text-gray-500 font-mono text-center">{time}</td>
                    {DAYS_OF_WEEK.map(day => {
                      const course = getCourseForSlot(day, time);
                      return (
                        <td key={`${day}-${time}`} className="border-b border-r border-gray-100 dark:border-gray-800 p-1 align-top h-16 relative">
                          {course && (
                            <div
          ...
          <div className="lg:hidden space-y-6">
            {DAYS_OF_WEEK.map(day => {
              ...
              return (
                <div key={day} className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 p-5 shadow-sm">
                  <h3 className="font-bold text-gray-900 dark:text-gray-100 mb-4 pb-2 border-b border-gray-100 dark:border-gray-800">{day}</h3>
                  <div className="space-y-3">
                    {dayCourses.map(course => (
                      <div key={course.id} className="flex gap-4 group">
                        <div className="w-16 pt-1 text-right">
                          <div className="text-sm font-bold text-gray-900 dark:text-gray-100">{course.startTime}</div>
                          <div className="text-xs text-gray-400 dark:text-gray-500">{course.endTime}</div>
                        </div>
                        <div
                        ...
                        >
                          <div>
                            <div className="font-bold text-gray-900 dark:text-gray-100" style={{ color: course.color }}>{course.name}</div>
                            <div className="text-sm text-gray-500 dark:text-gray-400 mt-1 flex items-center gap-1">
                              <MapPin size={12} />
                              {course.location}
                            </div>
                          </div>
                          <button onClick={() => handleDeleteCourse(course.id)} className="text-gray-300 hover:text-red-500 dark:text-red-400 p-1">
                            <Trash2 size={16} />
                          </button>
                        </div>
                      </div>
            ...
            {courses.length === 0 && (
              <div className="text-center py-12 bg-gray-50 dark:bg-gray-900 rounded-2xl border border-dashed border-gray-200 dark:border-gray-700">
                ...
                <Button variant="link" onClick={() => setShowAddForm(true)} className="text-indigo-600 dark:text-indigo-400">Add your first class</Button>
              </div>
            )}
          </div>
